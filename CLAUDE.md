...
- **主题候选从真实提问识别，不从已发布页面事后聚类**（2026-08-03 修订，设计依据 `docs/design/wiki.md`「主题：从真实使用中识别，而不是从已发布词条事后聚类」，取代此前"对已发布概念页求连通分量"的口径）：Study 对 `traces` 按归一化四元组 `(subject, intent, audience, constraint_text)` 分组，分组同时满足 `distinct_question_count ≥ wiki.topic_cluster_min_questions`、`days_active ≥ wiki.topic_cluster_min_days_active` 才是主题候选——此刻不涉及任何知识点，与材料侧是否已有已发布页面无关。候选范围确定后才在其中检索知识点、筛出**主题范围材料**（`lifecycle=current`，**不**要求 verified——2026-08-04：手动/主题范围先出草稿，verified 留给一阶材料 qualifying、ready、二阶可靠度与正式发布）、按 entry_id 分组：已发布概念页直接复用，未发布但满足概念级 ready 判定的分组随批推进一阶编译。二阶编译准入回答两件事：**关联**（成员概念页之间 related 连接为主导、contradicts 不反客为主）与**整体可靠度**（候选范围全部知识点的 verified 覆盖占比达标），不再对页面图求连通分量、不设候选创建时的成员数硬性区间。`wiki_pages.uncovered_points`（current 但无 verified 链接的 KP）**只作字段、不进正文**——正文四节/五节结构与 citation 白名单校验一条不动。`internal/study/`、`internal/wiki/` 中的连通分量实现尚未跟进这次修订，编码前应以 `docs/impl/v1/wiki.md` 步骤 8 为准重新核对，不要照搬旧实现。
