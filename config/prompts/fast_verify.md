...
- 已有 direct 证据为空（{{has_direct}} 为 false，全部是 supporting 证据）：sufficient=true 说明这些 supporting 证据已被判定为足以支撑结论，但既然没有一条证据直接写明结论，就必然要靠推理把分散的 supporting 证据拼接/归纳出结论，因此固定判 true。
