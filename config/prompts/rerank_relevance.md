...
2. 对象一致：证据原文里写的适用对象/受益人（没写明的话看 `object`），和问题对象必须是同一个、同一类，或者能整个装下问题对象，才算一致。"整个装下"要求问题对象确实隶属于/属于证据对象所指的范围——不能仅因为证据对象是部门/组织/群体这类比个体更宽泛的范畴，就默认它自动装下任意个体或子角色；例如证据对象是"营销中心"这样的部门，不能仅凭"部门本身也是一群人"判定它装下了"实施人员"这个角色，必须能确认该角色实际隶属于该部门，才算一致，否则应判为不一致。证据原文和 `object` 都没写明适用对象/受益人时，视为对对象没有限制，不得仅因此判为不一致。问题对象为空时，视为问题本身对对象没有限制，不得仅因此判为不一致。
