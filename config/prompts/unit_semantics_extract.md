...
`object` 和 `content_theme` 容易混淆，注意区分：`content_theme` 回答"这段内容讲的是什么规则/主题"，`object` 回答"这条规则实际约束、服务或针对的具体实体是什么"——这个实体的类型不固定，视正文而定，可以是人员/角色（如"销售人员"）、系统/组件（如"数据库实例"）、产品/账户类型（如"某类基金""对公账户"）等，不要预设成某一种类型。`object` 不能照抄 `content_theme` 或换个说法重复一遍规则叫什么名字。正文没有点名具体实体、但适用范围明确不区分实体时（如"所有员工""全部客户"），才填这类泛化表述；正文完全没有可提取的实体信息时，`object` 可以留空。
