...
`merged_into` 非空的概念在以下位置一律排除：`unit_entry_match` 的候选概念列表、Wiki 候选计算（study 步骤 6）、Page 的概念列举。历史数据不迁移：既有 KU 指向被合并概念的 `entry_id` 在合并事务中统一改写（见步骤 3）。

**preset UPSERT 规则调整**（foundation 启动加载）：按 entry_id UPSERT 时只更新 name / description，**不得**清除 `merged_into`、不得改写 `origin`。被合并的概念不因 preset 中仍存在而恢复为当前入口。
...
{ "question_terms": "...", "direct_point_ids": ["..."],
  "gap_level": "link_gap | entry_gap", "null_entry_ratio": 0.8 }
```

**判定时机与规则**（trace_write 组装 payload 时，一次数据库 join，不增加 LLM 调用）：

```text
direct_point_ids -> KP -> 所属 KU -> entry_id；
无概念归属（entry_id 为 NULL，或其概念 merged_into 非空）的 KP 占比
  ≥ entry_null_ratio_min -> gap_level = entry_gap；
...
修改 trace 模块 `activation_gap` 事件的 payload 组装，按上述规则写入 `gap_level` 和 `null_entry_ratio`。此改动随本模块 migration 一起上线，早于 Study 扫描启用。
...
```text
取窗口内 gap_level = entry_gap 且未 processed 的事件；
贪心聚簇：按 KP 集合 Jaccard ≥ entry_add_overlap_min 并簇；
簇达标（事件数 / 不同问题数 / 重叠度）->
  创建或更新 entry_candidates(kind=add)：
    point_ids   = 簇内 KP 并集；
    domain_id   = 簇内 KU 所属 source 的 domain_id 多数决（可为 NULL）；
    suggested_name = 簇内 KU center 分词后的高频词组合
                     （程序统计，不调 LLM，确认时人工可改）；
  写 learning_results(action=entry_add_candidate,
    object_type=entry_candidate, status=pending_confirm)；
...
```text
取窗口内 traces 的 cited point_ids -> KU -> entry_id，
  对每次问答提取涉及的概念对（去重）；
概念对共现计数 ≥ entry_merge_cooccur_min 时，
  计算共同采用中的 KP 重叠比例；
两条件同时达标 -> entry_candidates(kind=merge, merge_from=[A,B])
  + learning_results(action=entry_merge_candidate)；
排除 merged_into 非空的概念参与统计。
```

**过期**：`last_signal_at` 距今超过 `entry_candidate_idle_days` 的 pending_confirm 候选置 expired，对应 learning_results 同步更新。

### 步骤 3：确认与执行 API

```text
GET  /entries/candidates?status=pending_confirm   列出候选（含完整 evidence）
POST /entries/candidates/:id/confirm              确认执行
POST /entries/candidates/:id/reject               拒绝
```
...
kind=add：
  ...
  INSERT entries(origin='evolved')；
  簇内 KP 所属 KU 中 entry_id 为 NULL 的行，UPDATE 为新概念；
  不自动创建 ActivationLink——新概念的激活路径走正常的
  candidate 链接形成与验证流程；

kind=merge：
  body 指定 target（并入 merge_from 中的哪一个）；
  被并概念下所有 KU 的 entry_id UPDATE 为 target；
...
```text
unit_entry_match（unit 模块步骤 5）：候选概念列表排除 merged_into 非空；
...
```text
Foundation：migration（entries 两列 + entry_candidates 表）、preset 规则调整
Trace：     activation_gap payload 扩展（步骤 1，本模块内改动）
Study：     Ticker 任务链扩展（步骤 2），learning_results 复用
Unit：      unit_entry_match 概念列表排除规则（步骤 4）
...
gap_level 判定正确：无概念归属占比达阈值的 activation_gap 带
  entry_gap，其余 link_gap；存量无字段事件按 link_gap 兼容消费；
新增聚类：fake LLM 下构造同主题 entry_gap 事件，事件数 / 不同问题数 /
...
confirm 执行：add 后新概念可被 unit_entry_match 选中且 origin=evolved；
