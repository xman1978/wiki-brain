...
func TestStore_CreateAndList(t *testing.T) {
	...
	if d.ConceptCount != 0 || d.SourceCount != 0 || d.KPCount != 0 || d.PendingSignalCount != 0 {
...
func TestStore_List_Counts(t *testing.T) {
	...
	mustExec(t, db, `INSERT INTO concepts (concept_id, domain_id, name) VALUES ('c1', ?, 'concept one')`, domainID)
	mustExec(t, db, `INSERT INTO concepts (concept_id, domain_id, name, merged_into) VALUES ('c2', ?, 'concept two', 'c1')`, domainID)
	...
	mustExec(t, db, `INSERT INTO knowledge_units (unit_id, source_id, concept_id, center, line_start, line_end, status, lifecycle, prompt_version)
	...
	mustExec(t, db, `INSERT INTO concept_candidates (candidate_id, kind, domain_id, suggested_name, status, last_signal_at)
	...
	d := domains[0]
	if d.ConceptCount != 1 {
		t.Errorf("concept_count = %d, want 1 (merged-away excluded)", d.ConceptCount)
	}
	if d.SourceCount != 1 {
