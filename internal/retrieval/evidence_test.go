...
func TestBuildEvidenceSet_WithMining_ProducesFragmentLevelEvidence(t *testing.T) {
	...
	es, err := svc.buildEvidenceSet(context.Background(), "what is a linear equation", "", "", "", "", "short", direct, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestBuildEvidenceSet_MiningDisabled_WholeSegmentUnchanged(t *testing.T) {
	...
	es, err := svc.buildEvidenceSet(context.Background(), "q", "", "", "", "", "short", direct, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
