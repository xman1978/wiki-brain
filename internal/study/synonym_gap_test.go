...
func TestAggregateSynonymCandidates_CreatesPendingConfirmWhenThresholdMet(t *testing.T) {
	...
	svc := NewService(store, testConfigWithSynonym(2, 2, false), activationSvc, nil, 0, 0, 0, false, 0, 0)
...
func TestAggregateSynonymCandidates_BelowThreshold_NoCandidate(t *testing.T) {
	...
	store := NewStore(db)
	activationSvc := newTestActivationSvc(db)
	svc := NewService(store, testConfigWithSynonym(3, 2, false), activationSvc, nil, 0, 0, 0, false, 0, 0)
...
func TestAggregateSynonymCandidates_AutoPromote_CreatesActiveDirectly(t *testing.T) {
	...
	store := NewStore(db)
	activationSvc := newTestActivationSvc(db)
	svc := NewService(store, testConfigWithSynonym(2, 2, true), activationSvc, nil, 0, 0, 0, false, 0, 0)
...
func TestAggregateSynonymCandidates_DedupSkipsExistingTerm(t *testing.T) {
	...
	store := NewStore(db)
	activationSvc := newTestActivationSvc(db)
	svc := NewService(store, testConfigWithSynonym(1, 1, false), activationSvc, nil, 0, 0, 0, false, 0, 0)
