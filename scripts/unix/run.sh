#!/usr/bin/env bash
# Wiki-Brain 服务启动/停止脚本（打包产物专用）
# 用法：./run.sh start | ./run.sh stop | ./run.sh restart | ./run.sh status
#
# 这是随 dist 包分发的版本，直接运行同目录下已编译好的 wiki-brain-server，
# 不做 go build（源码版本的等价脚本是仓库根目录的 run.sh，那个版本每次
# start 都会重新编译）。
...
BIN_PATH="$ROOT_DIR/wiki-brain-server"
...
# 端口被占用、但占用者不是我们自己的二进制——这种情况下不能当成"服务已经
# 在运行"直接跳过，也不能硬启动（启动了也绑不上端口），要明确报错让人去
# 处理端口冲突，而不是等 bind 失败才发现。
port_conflict() {
    local pid
    pid="$(port_pid)"
